...
public class Machine {
	...
	public int tempoMaq(ReadInstances arq, int nummaq, ArrayList<ArrayList<Integer>> tempos,
			ArrayList<Integer> pos_alter, int ms_atual) {
	...
	public void setJobMaq(int pos, int job) {
		...
		this.tempo_maq = this.tempo_maq + (-1) * t_prep_antigo + t_prep_novo_antes + t_prep_novo_depois + tempo_exec;
	}
	...
	public void removeJobToMaq(int pos) {
		...
		if (pos > 0) {
			...
			if (pos == t - 1) {
				...
				t_prep_antigo_depois = this.arquivo.getT_prep(position, this.machine.get(pos),
						this.machine.get(pos + 1));
		...
		this.tempo_maq = this.tempo_maq + t_prep_novo + (-1) * t_prep_antigo_depois + (-1) * t_prep_antigo_antes
				+ (-1) * tempo_exec;
	...
	public void trocaJob(int i, int j) {
		...
		if (i == 0) {
			...
			t_antigo_antes_i = this.arquivo.getT_prep(position, this.machine.get(i - 1), this.machine.get(i));
			t_novo_antes_i = this.arquivo.getT_prep(position, this.machine.get(i - 1), this.machine.get(j));
		}
		...
		if (j + 1 == t) {
			...
			t_antigo_depois_j = this.arquivo.getT_prep(position, this.machine.get(j), this.machine.get(j + 1));
			t_novo_depois_j = this.arquivo.getT_prep(position, this.machine.get(i), this.machine.get(j + 1));
		}
		...
		if (j - i == 1) {
			t_antigo_depois_i = 0;
			t_novo_antes_j = 0;
			t_novo_depois_i = this.arquivo.getT_prep(position, this.machine.get(j), this.machine.get(i));
			t_antigo_antes_j = this.arquivo.getT_prep(position, this.machine.get(j - 1), this.machine.get(j));
		}
		// trata a situação comum resultade
			...
			if (i == t - 1) {
				...
				t_antigo_depois_i = this.arquivo.getT_prep(position, this.machine.get(i), this.machine.get(i + 1));
				t_novo_depois_i = this.arquivo.getT_prep(position, this.machine.get(j), this.machine.get(i + 1));
			}
			if (j == 0) {
				t_antigo_antes_j = 0;
				t_novo_antes_j = 0;
				...
				t_antigo_antes_j = this.arquivo.getT_prep(position, this.machine.get(j - 1), this.machine.get(j));
				t_novo_antes_j = this.arquivo.getT_prep(position, this.machine.get(j - 1), this.machine.get(i));
			}
		}

		this.tempo_maq = this.tempo_maq + (-1) * t_antigo_antes_i + t_novo_antes_i + (-1) * t_antigo_antes_j
				+ t_novo_antes_j + (-1) * t_antigo_depois_i + t_novo_depois_i + (-1) * t_antigo_depois_j
				+ t_novo_depois_j;
